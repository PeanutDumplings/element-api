...
export async function GET(request: NextRequest, { params }: { params: { block: string } }) {
    try {
        const block = params.block.toLowerCase();
        ...
        if (!limit) {
            ...
            if (!query.length) {
                return NextResponse.json(
                    {
                        success: false,
                        message: `No elements with block '${block}' found`,
                        block,
                        validBlocks: [
                            "nonmetal",
                            "alkali metal",
