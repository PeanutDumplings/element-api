...
export async function GET(request: NextRequest) {
    try {
        ...
        let query: any;
        if (!limit) {
            query = await getXataClient().db.elements.select(fetch).getAll();
                ...
                return NextResponse.json(
                    {
                        success: false,
                        message:
                            "No elements found. This is definitley an error. Please open an issue @ https://github.com/PeanutDumplings/element-api/issues",
                    },
