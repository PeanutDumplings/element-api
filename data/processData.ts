...
import jsonData from "./input/elements_data.json";
...
jsonData.forEach((element) => {
    element.electronicConfiguration = electronsInSubshell(element.electronicConfiguration);
});

// Round electronegativity to 2 decimal places
jsonData.forEach((element) => {
    element.electronegativity = isNaN(parseFloat(element.electronegativity.toString()))
        ? ""
...
const outputFile = path.join(outputDir, "processed_elements_data.json");
